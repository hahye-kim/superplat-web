...
## v60 — NEW HOME + 구 HOME → ABOUT 분리

### route

| 이전 | 이후 |
|---|---|
| `index.html` (스토리텔링 홈) | `about.html` — **내용 그대로** 이동 |
| — | `index.html` — 새 HOME |

전 페이지의 `index.html#about` → `about.html`. 로고 → `index.html`.
`index.html#download` 는 새 홈의 Final CTA 가 같은 `id="download"` 를 가져 그대로 동작합니다.

### 새 HOME 구성

```
01 HERO (video)  →  02 CHARACTER  →  03 WORLD rail  →  04 NEWS  →  05 FAQ  →  06 CTA
```

- **Hero** — ABOUT 하단과 **같은 영상 파일**(`superplat-video.mp4`, ≤900px 는 `-720`).
  새 영상 없음. `autoplay muted loop playsinline`. object-position PC 42% / 모바일 38%.
- **Character** — 4장 자동 슬라이더(4.6s). 이미지·제목·설명이 한 인덱스로 묶여 움직입니다.
  hover 정지 · prev/next · swipe · 조작 후 8s 재개 · 4분할 진행 표시.
- **World rail** — 19개 공간의 seamless infinite rail(22px/s). 목록 2벌 + wrap 방식이라
  첫↔마지막 경계에서 점프가 없습니다. hover 감속 · drag/swipe · 1.2s 뒤 복귀.
- **NEWS / FAQ** — 아래 공유 데이터 모듈에서 그립니다.
...
| 신규 모듈 | 쓰는 화면 |
|---|---|
| `assets/sp-news.js` | 홈 NEWS · NEWS 페이지 |
| `assets/sp-faq.js` | 홈 FAQ · 고객지원 FAQ |
| `assets/sp-worlds.js` (개편) | 홈 rail · ABOUT 캐러셀 · WORLD & IP |

`sp-worlds.js` 에 `type`(공간 유형)을 추가하고, **확정 리스트 19개**의 이름 ↔ 유형 ↔ 이미지로
전면 갱신했습니다. 세 화면이 같은 배열을 보므로 한 곳만 고치면 모두 반영됩니다.

⚠️ `Theater Hall` · `Private Gallery` 의 설명 문구(`ko`)는 미수령이라 비어 있습니다.
WORLD & IP 카드는 공용 기본 문구로 대체 표시하며, 문구를 지어내지 않았습니다.

### 추가 asset (8개, 전부 프로젝트 내부)

`home-char-supertown/ai/social/cafe.jpg` · `home-char-crew.jpg` ·
`space-busan-museum.jpg` · `space-private-gallery.jpg` · `space-pop-up-store.jpg`

원본 PNG(합계 3.7MB)를 **JPEG q97 · 4:4:4 무손실 서브샘플링**으로 변환했습니다(1.5MB).
크롭 · 비율 · 구도는 그대로이고, 평균 색차는 1~2/255 입니다.

### 실측으로 잡은 버그 3건

| 증상 | 원인 | 조치 |
|---|---|---|
| Dark 에서 CTA 버튼이 흰 배경 + 흰 글씨 | `background:var(--ink); color:#fff` — Dark 에서 `--ink` 가 밝아짐 | `sp-theme.css` 의 solid-ink 버튼 목록에 3종 추가 |
| 마우스 드래그가 18px 만 이동 | 카드가 `<a>` 라 브라우저가 네이티브 링크 드래그를 시작 → `pointercancel` | `draggable=false` + `dragstart` preventDefault |
| 드래그가 첫 프레임에 끊김 | `setPointerCapture` 호출 시 `pointerleave` 가 한 번 발생 | `pointerleave` 로 endDrag 하지 않음 |

### QA (실측)

| 항목 | 결과 |
|---|---|
| 기능 36항목 (Hero · 슬라이더 · rail · NEWS · FAQ · CTA) | ✅ ALL PASS |
| 19개 공간 이름·유형·이미지 데이터 일치 | ✅ 19/19 |
| 16:9 비율 · radius · caption baseline 통일 | ✅ 1.78 / 24px / 동일 |
| 무한 루프 점프 | ✅ 0회 |
| hover 감속 · mouse drag · autoplay 복귀 | ✅ 1초 2.7px / −220px / 복귀 확인 |
| mobile touch swipe (430·390·375·360) | ✅ 4/4 |
| 보이는 카드 수 | PC 4.5 · 1024 3.7 · 768 2.8 · 모바일 1.6 |
| 반응형 11뷰포트 × Light/Dark × KO/EN | ✅ overflow 0 · 깨짐 0 · 오류 0 |
| 영상/이미지 위 흰 텍스트 대비 (글리프 픽셀 실측) | ✅ 최저 3.34:1 (Hero 5.77~7.62) |
| prefers-reduced-motion | ✅ rail 정지 · 콘텐츠 정상 |
| 사이트 전체 10페이지 | ✅ overflow 0 · 깨짐 0 · JS 0 · 404 0 |

Hero overlay 는 실측 2.60:1 이 나와 가운데만 강화했습니다(가장자리는 그대로 열려 있습니다).

---

## v61 — HOME 디테일 QA (4건)

### 1. Header — 첫 페인트부터 노출 (버그 수정)

새 홈에 `.js .header{opacity:0}` 가 남아 있었습니다. 예전 홈의 sticky hero 인트로가
`.is-ready` 에서 헤더를 켜 주던 구조인데, 새 홈에는 그 인트로가 없어 **3.5초 페일세이프가
붙을 때까지 헤더가 통째로 투명**했습니다(실측). opacity 게이트와 죽은 `.shero__*` 인트로
규칙을 걷어내고 `.header{opacity:1}` 로 고정했습니다.
Hero 위 transparent / 스크롤 후 solid 전환은 그대로입니다.

### 2. WORLD rail — 좌우 gradient fade

`.hworld__rail::before/::after` 로 양 끝에 fade 를 넣었습니다.
색은 `--home-world-bg`(= `--bg`) 라 Light/Dark 를 따라갑니다.

| | fade 폭 | 카드폭 대비 |
|---|---|---|
| 1920 / 1440 | 134 / 101px | 33% |
| 1024 / 768 | 80 / 48px | 31% / 18% |
| 430 ~ 360 | 30 ~ 25px | 12% |

`pointer-events:none` + 절대 배치라 drag/swipe·클릭·layout 폭에 영향이 없습니다
(fade on/off 로 rail·track·body 폭을 재서 전부 동일한 것을 확인).

### 3. NEWS — 카테고리별 카드 슬롯 통일

- 이미지 비율을 기존 NEWS 카드(`.ecard__clip`)와 같은 **16:9** 로 맞춤
- 요약 줄을 **내용이 없어도 항상 렌더**하고 2줄 자리를 고정 (`min-height:calc(2*1.6em)`)
  — 공지·업데이트에는 요약이 없고 이벤트에는 기간이 들어가서, 이전에는 탭을 바꾸면
  카드 높이가 달라졌습니다
- `.hncard{height:100%}` + 날짜 `margin-top:auto`

실측: 공지/업데이트/이벤트 3탭 모두 **1440 → 386px · 1024 → 326px · 390 → 396px** 로 동일.
radius · padding · 날짜 baseline 도 전부 일치.

### 4. FAQ — 초기 상태 전부 닫힘

첫 항목 기본 열림을 제거했습니다. 초기 `aria-expanded="false"` × 5, 패널 높이 0.
클릭 시 열림 / 재클릭 시 닫힘, 키보드 Enter·Space, `aria-controls`, focus 상태는 그대로입니다.

### QA (실측)

39개 항목 ALL PASS — Header 첫 페인트(1440·390) · Hero 위 transparent · 스크롤 후 solid ·
rail fade 존재/색/pointer-events/폭 (Light·Dark) · fade 위에서 drag · NEWS 3탭 × 3뷰포트
높이·비율·radius·padding·baseline · FAQ 초기닫힘/열림/닫힘/키보드 (1440·390) ·
반응형 10뷰포트 × Light·Dark overflow 0 · 깨짐 0 · JS 오류 0.

---

## v62 — 고객지원 1:1 문의: 수정 / 삭제 + 첨부파일 확인

두 요청을 한 버전으로 묶었습니다. 대상 파일은 `support.html` 과
공용 prev/next 컴포넌트(`assets/sp-pn.js` · `assets/sp-pn.css`),
그리고 Dark 토큰(`assets/sp-theme.css`) 뿐입니다.
문의 목록 디자인 · 상세 구조 · 타이포 · 색 토큰은 건드리지 않았습니다.

### 1. 권한 정책을 별도 블록으로 분리

백엔드 정책이 아직 확정되지 않았다는 점을 반영해, 조건을 UI 코드 안에
흩뿌리지 않고 네 개의 함수로 모았습니다. 정책이 바뀌면 이 블록만 고치면 됩니다.

```js
function isMyInquiry(q)   { return !!uid && q.userId === uid; }
...
판정 기준은 `currentUser.id === inquiry.userId` 하나뿐입니다.
실제 API 연결 시 **서버에서도 같은 검증이 반복되어야 한다**는 주석을 함께 남겼습니다.

| 케이스 | 상태 | 작성자 | 결과 |
|---|---|---|---|
| id 1 | 답변중 | 본인 | 수정 · 삭제 노출 |
| id 2 | 답변중 | 타인 | 버튼 없음 |
| id 3 | 답변완료 | 본인 | 버튼 없음 + 안내 문구 |

### 2. 액션 영역 — 공용 컴포넌트 확장 (새 스타일 없음)

`[목록으로]` 한 줄만 그리던 `SPPrevNext` 에 **선택적 action slot** 을 더했습니다.
`actions` 를 넘기지 않는 페이지(NEWS 공지 · 업데이트 · 이벤트 상세)는
DOM·클래스·정렬이 이전과 완전히 동일합니다 — 회귀 테스트로 확인했습니다.
...
버튼은 기존 `.pn__list` 를 그대로 쓰고, 삭제만 `--danger` 변형을 붙였습니다.

### 3. 확인 팝업 · 토스트 — 마이페이지 컴포넌트 재사용

`.mpdlg` / `.mptoast` 클래스명을 그대로 사용했습니다.
새 이름을 만들면 Dark 대응을 처음부터 다시 써야 하는데, 이 이름들은
`sp-theme.css` 에 이미 Dark override 가 있어서 **재사용이 곧 Dark 지원**입니다.

- 수정: "문의를 수정하시겠습니까?" → "문의가 수정되었습니다."
- 삭제: "문의를 삭제하시겠습니까?" / "삭제한 문의는 복구할 수 없습니다."
  → `[취소] [삭제]` → "문의가 삭제되었습니다." → 목록 복귀

경고 톤은 버튼 하나에만 두고 배경·아이콘·테두리는 평소 그대로입니다.

### 4. 첨부파일 확인

핵심은 업로드 기능을 새로 만드는 게 아니라, **등록할 때 첨부한 사진을
나중에 다시 열었을 때 상세에서 확인할 수 있게** 하는 것입니다.
그래서 폼과 상세가 같은 데이터를 봅니다 — 임시 이미지를 끼워 넣지 않았습니다.

```js
attachments: [{ id, name, type, size, url }]   // url = FileReader data URL
```

- 배치: **문의 내용 → 첨부파일 → 고객지원 답변**, 첨부가 없으면 영역 자체를 렌더하지 않음
- 그리드: `repeat(auto-fill, minmax(clamp(96px,13vw,132px), 1fr))` — 고정 폭 없음
- 썸네일 1:1 `object-fit:cover`, 파일명은 아래에 ellipsis
- 수정 폼은 기존 첨부를 그대로 들고 시작하고, 사용자가 지우지 않는 한 저장 후에도 유지
- 문의를 삭제하면 첨부도 함께 사라짐 (같은 객체에 들어 있음)

### 5. 확대 보기 — WORLD 라이트박스와 같은 구조

`world.html` 의 `.wlb` 를 그대로 따라 `.alb` 를 만들었습니다.
...
```
   ...
   [<]  │                      │  [>]
        └──────────────────────┘
   world-freeze-01.jpg              1 / 2
```

ESC · ← → · 모바일 스와이프 · Tab focus trap · body 스크롤 잠금,
닫으면 눌렀던 썸네일로 포커스 복귀.

### 6. Dark — `.sbtn` 규칙에 가려져 있던 danger 색 (수정)

Dark 에서 삭제 확인 버튼이 빨강이 아니라 일반 확인 버튼과 똑같이 보였습니다.

```
html[data-theme="dark"] .sbtn  (0,2,1)  ← 이 규칙이
.sbtn--danger                  (0,1,0)  ← 이 규칙을 specificity 로 덮고 있었음
```

이번 프로젝트에서 반복해서 나온 "Dark override 가 없는 색" 계열의 문제입니다.
이미 같은 방식으로 해결돼 있던 `.abtn--danger` 패턴에 맞춰 override 를 추가하고,
`.pn__list--danger` 의 리터럴 테두리도 Dark 토큰으로 맞췄습니다.

| | Light | Dark |
|---|---|---|
| `[삭제]` 텍스트 | `#C0392B` · 5.44:1 | `#FF8A80` · 8.01:1 |
| 확인 `[삭제]` | 흰 글자 / `#C0392B` · 5.44:1 | `#2A0F0C` / `#FF8A80` · 7.85:1 |

### QA (실측)

- **1:1 문의 수정/삭제** — 전 항목 PASS. 15단계 플로우 · 권한 3케이스 ·
  7개 뷰포트 × 테마/로케일 조합(액션 레이아웃 최소 156×52 · 겹침 0 · overflow 0)
- **첨부파일** — 전 항목 PASS. 비율이 다른 3장(1600×900 · 600×1000 · 900×900)을
  올려 `contain` 비율 유지와 화면 안 수납을 1440/1024/768/430/390/375/360 에서 확인.
  모바일 라이트박스 버튼 44×44 이상
- **Dark 대비** — 첨부 라벨 16.88:1 · 파일명 · 액션 3버튼 · 라이트박스 21.00:1 ·
  다이얼로그 제목/본문/버튼 전부 4.5:1 이상
- **공유 컴포넌트 회귀** — NEWS 공지·업데이트·이벤트 상세에서
  `backwrap--split` 미적용 · action slot 0 · danger 0 · 버튼 1개 · 라벨 "목록으로" ·
  prev/next 카드 정상, 모바일 overflow 0. WORLD 도 변화 없음
- **사이트 전체** — 10개 페이지 overflow 0 · 깨진 이미지 0 · JS 오류 0 · 404 0,
  WORLD 카드 19장과 상세 뷰어 19/19 정상

### v62.1 — 첨부파일 ↔ 고객지원 답변 section gap

마지막 파일명 바로 아래에 답변 박스가 붙어 있어 두 영역이 한 덩어리로 읽혔습니다.
**섹션 간 간격만** 띄웠습니다 — 썸네일 크기 · 이미지 간격 · 파일명 간격 ·
답변 박스 디자인 · 상세 레이아웃은 그대로입니다.

```css
.atts{margin-bottom:48px}                              /* PC */
@media (max-width:1024px){ .atts{margin-bottom:40px} } /* Tablet */
@media (max-width:767px){  .atts{margin-bottom:32px} } /* Mobile */
```

기존 `--space-content-lg`(36/40/56/72)를 먼저 검토했는데 **태블릿 40 만 맞고
PC 가 72 로 과해서** 이 구간 전용 값을 두되, 프로젝트가 이미 쓰는
breakpoint(1024 · 767)에 맞췄습니다. 1024 를 태블릿 쪽에 넣은 이유는
`--space-content-lg` 자체가 1024 에서 40px 로 계산되어, 디자인 시스템이
이미 1024 를 태블릿 스페이싱 밴드로 보고 있기 때문입니다.

실측 — 11개 뷰포트에서 첨부 영역 하단 ↔ 답변 박스 상단:

| | 1600 · 1440 · 1280 | 1024 · 991 · 834 · 768 | 430 · 390 · 375 · 360 |
|---|---|---|---|
| 간격 | 48px | 40px | 32px |

불변 확인: 썸네일 133/146/110px · 이미지 간격 `clamp(10px,1vw,14px)` ·
파일명 간격 7px · 라벨→그리드 12px · 답변 박스 padding/radius · overflow 0.
첨부가 없는 문의는 영역 자체가 없으므로 본문↔답변 간격(본문 padding-bottom)도 그대로입니다.
확대 보기 · ← → 이동 · 수정 폼 첨부 유지 · 삭제 확인 팝업 전부 정상.

### v62.2 — 1:1 문의 상세 Action Bar

모바일이 `[수정][삭제]` 위 / `[목록으로]` 전체 폭 아래의 2단이라, 목록 버튼만
CTA 처럼 커 보이고 세 액션의 관계가 드러나지 않았습니다.
**PC · 태블릿 · 모바일이 같은 한 줄 구조**를 쓰도록 바꿨습니다.

```
[ 목록으로 ]                              [ 수정 ] [ 삭제 ]
 Navigation                        Secondary   Destructive
```

Breakpoint 마다 다른 UI 를 만들지 않았습니다. 좁아지면 배치가 아니라
`padding-inline` 과 `gap` 만 줄어듭니다 — **320px 까지 한 줄이 유지**돼서
결국 fallback 규칙을 넣을 필요가 없었습니다(실측으로 확인 후 제거).

```css
.backwrap--split{justify-content:space-between;align-items:center;
  flex-wrap:nowrap;gap:clamp(10px,1.6vw,20px)}
.backwrap--split .pn__list{width:auto;max-width:none;flex:0 0 auto;
  padding-inline:clamp(14px,1.5vw,22px);white-space:nowrap}
.backwrap--split > .pn__list{min-width:clamp(84px,7.6vw,108px)}   /* 목록으로 */
.backwrap__acts .pn__list{min-width:clamp(72px,6.2vw,92px)}       /* 수정 · 삭제 */
```

세 버튼 모두 기존 `.pn__list` 그대로라 height · radius · font-weight · border ·
hover · focus · transition 이 자동으로 같습니다. 폭만 콘텐츠에 맞췄고,
새 배경 카드나 divider 는 두지 않았습니다.

**권한에 따라 버튼이 사라져도 위치가 고정됩니다.** `actions` 가 비면 이전에는
split 이 풀려 `[목록으로]` 가 가운데 전체 폭 CTA 로 바뀌었는데, `align:'split'`
옵션을 추가해 명시적으로 유지합니다. 실측: 답변완료·타인 문의 모두 좌측 x 좌표가
액션이 다 있을 때와 동일(1440 → 58px · 390 → 20px).

`assets/sp-pn.js` 는 `acts.length > 0 || o.list.align === 'split'` 한 줄만 바뀌었고,
둘 다 없는 NEWS 공지·업데이트·이벤트 상세는 DOM·클래스·정렬이 그대로입니다.

### QA (실측)

10개 뷰포트(1920·1440·1280·1024·768·430·390·375·360·320) × Light/Dark:

| | PC 1440 | 1280 | 1024 · 768 | 모바일 |
|---|---|---|---|---|
| 목록으로 | 108px | 97px | 84px | 84px |
| 수정 · 삭제 | 89px | 79px | 72px | 72px |
| 높이 | 48px | 48px | 48px | 52px |

전 구간 **행 1 · 겹침 없음 · overflow 0 · 높이 3개 동일 · 텍스트 중앙 정렬 ·
Touch Target 48~52px**. 영문 로케일(`Back to list / Edit / Delete`)도 320px 까지 한 줄.

삭제 버튼: 투명 배경 · red 텍스트 · soft red 보더 유지(solid red 아님),
Light 5.44:1 · Dark 8.01:1, hover `rgba(192,57,43,.06)`, focus outline red.
「다음 문의」 카드 ↔ Action Area 40px.

무영향 확인: 수정 진입 시 첨부 유지 · 삭제 Confirm Modal · 첨부 확대보기 ·
권한 정책 · NEWS/WORLD 공유 컴포넌트 · 사이트 전체 10페이지 · JS 오류 0.
